...
import { createPostgresPdo, PostgresDriver } from '../../src';
...
describe('Postgres Driver', () => {
  ...
  it('Works Driver Registration', () => {
    expect(Pdo.getAvailableDrivers()).toEqual([
      'pg',
      'pgsql',
      'crdb',
      'cockroachdb',
  ...
  it('Works Driver Notification', async () => {
    if (isCrdb()) {
      ...
      expect(PostgresDriver.subscribe('test_channel', fnTestChannel)).toBe(
        true,
      );
      expect(PostgresDriver.subscribe('test_channel', fnTestChannel)).toBe(
        false,
      );
      expect(PostgresDriver.subscribe('test_channel2', fnTestChannel2)).toBe(
        true,
      );
      expect(PostgresDriver.subscribe('test_channel2', fnTestChannel2)).toBe(
        false,
      ...
      expect(PostgresDriver.unsubscribe('test_channel', fnTestChannel)).toBe(
        true,
      );
      expect(PostgresDriver.unsubscribe('test_channel2', fnTestChannel2)).toBe(
        true,
      );
      expect(PostgresDriver.unsubscribe('test_channel', fnTestChannel3)).toBe(
        false,
      );
    }
  });
