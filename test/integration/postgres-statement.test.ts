...
describe('Postgres Statement', () => {
  ...
  it('Works Statement Last Insert Id', async () => {
    ...
    stmt = await trx.query(
      "INSERT INTO users (name, gender) VALUES ('Claudio', 'All') returning *;",
    );
    expect(await stmt.lastInsertId()).toBeGreaterThan(lastId);
    expect(await stmt.lastInsertId()).toBe(
      await stmt.lastInsertId('test_db_users_id_seq'),
    );
    expect(stmt.fetchColumn(0).get()).toBe(await stmt.lastInsertId());
    await trx.rollback();
    await pdo.exec(
      'CREATE TABLE test (id serial, name VARCHAR(255) NOT NULL);',
    );
  ...
  it('Works Statement Last Insert Id Throw Error', async () => {
    ...
    await expect(stmt.lastInsertId()).rejects.toThrow(
