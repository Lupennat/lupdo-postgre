...
describe('Utils', () => {
  it('Works Sql Question Mark To Numeric Dollar', () => {
    expect(sqlQuestionMarkToNumericDollar('SELECT ?,?,?,?')).toBe(
      'SELECT $1,$2,$3,$4',
    );
    expect(sqlQuestionMarkToNumericDollar('SELECT "?,?,?,?"')).toBe(
      'SELECT "?,?,?,?"',
    ...
    expect(sqlQuestionMarkToNumericDollar("/*SELECT '??' */ SELECT ?")).toBe(
      'SELECT $1',
    );
    expect(
      sqlQuestionMarkToNumericDollar(
  ...
  it('Works Parse Bigint', () => {
    ...
    expect(parseBigint('-9007199254740992')).toEqual(
